...
### Fixed
- `fmtPL` reused the dollar amount's `+`/`-` sign for the percentage too,
  instead of giving the percentage its own sign. Usually harmless since both
  numbers normally agree, but the portfolio's Movement % and Movement $ are
  now computed from different subsets of stocks (see 0.12.0), so they can
  legitimately disagree in sign — producing a stacked "+-12.73%" when the $
  total was positive but the %'s costed subset was negative.
